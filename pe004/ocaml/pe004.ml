...
let reverse_int n =
  let rec loop n rev =
    if n = 0 then rev
    else loop (n / 10) ((rev * 10) + (n mod 10))
  in
  loop n 0
;;
...
let pe004 () =
  let rec loop a b largest =
    if a < 100 then largest
    else
      match (b < a, (a * b) < largest) with
      | ( true,     _) -> loop (a - 1) 999 largest
      | (false,  true) -> loop (a - 1) 999 largest
      | (false, false) -> if is_palindrome (a * b) then
			    loop a (b - 1) (a * b)
			  else
			    loop a (b - 1) largest
  in
